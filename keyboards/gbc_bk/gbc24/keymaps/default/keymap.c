...
enum custom_keycodes {
    JOY_TOG0 = SAFE_RANGE,  // レイヤー0 アナログ/デジタル切り替え
    JOY_TOG1,               // レイヤー1
    JOY_TOG2,               // レイヤー2
    JOY_TOG3,               // レイヤー3
};
...
joystick_config_t joystick_axes[JOYSTICK_AXIS_COUNT] = {
    JOYSTICK_AXIS_VIRTUAL,  // 軸0: X軸
    JOYSTICK_AXIS_VIRTUAL,  // 軸1: Y軸
};
...
static uint32_t analog_layer_mask = 0;

void joy_set_analog_layer(uint8_t layer) {
    analog_layer_mask |= ((uint32_t)1 << layer);
}

void joy_unset_analog_layer(uint8_t layer) {
    analog_layer_mask &= ~((uint32_t)1 << layer);
}
...
#ifdef JOYSTICK_ENABLE 
    joy_set_analog_layer(2);
    joy_set_analog_layer(3);
#endif 
}
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        switch (keycode) {
            ...
            case JOY_TOG2:
            case JOY_TOG3: {
                uint8_t layer = keycode - JOY_TOG0;  // 0～3を算出
                if ((analog_layer_mask >> layer) & 1) {
                    joy_unset_analog_layer(layer);
                } else {
                    joy_set_analog_layer(layer);
                }
                return false;
            }
#endif
...
void housekeeping_task_user(void) {
    ...
    bool is_analog = (analog_layer_mask >> get_highest_layer(layer_state)) & 1;
    if (is_analog) {
        if (last_dir != DIR_C) {
            send_dir_key(last_dir, false);
            last_dir = DIR_C;
        }
        int16_t ax = ((int16_t)x - JOY_X_CENTER) * 127 / 512;
        int16_t ay = ((int16_t)y - JOY_Y_CENTER) * 127 / 512;
        joystick_set_axis(0, (int8_t)CLAMP(ax, -127, 127));
        joystick_set_axis(1, (int8_t)CLAMP(ay, -127, 127));
        return;
    }
    joystick_set_axis(0, 0); joystick_set_axis(1, 0);
